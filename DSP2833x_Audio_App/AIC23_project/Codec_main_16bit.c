...
Uint16 ping_buffer[2];  	// Note that Uint32 is used, not Uint16
...
Uint16 * ch1_ptr; // ptr that points to ping/pong buffer that just received data

Uint32 L_buff[512]; // left channel audio circular buffer
Uint32 R_buff[512]; // left channel audio circular buffer
Uint32 LR_buff_size = 512; // size of both channel audio buffers

Uint32 * L_ptr = &L_buff[0];   // global pointer for L-channel buffer
Uint32 * R_ptr = &R_buff[0];   // global pointer for R-channel buffer
Uint32 * Ldelptr = &L_buff[0]; // global delay pointer
Uint32 * Rdelptr = &R_buff[0]; // global delay pointer

Uint16 first_interrupt = 1;   // 1 indicates first interrupt
Uint32 k = 0;  // idk what this is
Uint16 r_flag1 = 0; // flag set in interrupt to que data process
Uint16 r_flag2 = 0; // flag set in interrupt to que data process
Uint16 delay = 20; // delay value for effect
...
/* Fill the buffer with dummy data */
	...
	for(k=0; k<p_buff_size; k++) { ping_buffer[k] = 0xDEAD; }
	for(k=0; k<p_buff_size; k++) { pong_buffer[k] = 0xDEAD; }
	for(k=0; k<LR_buff_size; k++) { L_buff[k] = 0x00000000; }
	for(k=0; k<LR_buff_size; k++) { R_buff[k] = 0x00000000; }
...
/* Wait for data */
  	...
  	while(1) {
  		...
  		if(r_flag1==1)
  		{
  			...
  			//McbspaRegs.DXR2.all = (Uint16)((0xFFFF0000 & *ch1_ptr) >> 16);
  			//McbspaRegs.DXR1.all = 0x0000FFFF & *ch1_ptr;

  			//*R_ptr = (int32)(*ch1_ptr) >> 1;
  			//ch1_ptr++;
  			//*L_ptr = (int32)(*ch1_ptr) >> 1;
  			//ch1_ptr--;
  			//Ldelptr = L_ptr-delay;
  			//Rdelptr = R_ptr-delay;
  			//if(Ldelptr < &L_buff[0])
  			//{
  			//	Ldelptr = &L_buff[0] + (&L_buff[0] - Ldelptr);
  			//}
  			//if(Rdelptr < &R_buff[0])
  			//{
  			//	Rdelptr = &R_buff[0] + (&R_buff[0] - Rdelptr);
  			//}
  			//*ch1_ptr = (*Ldelptr/2) + (*L_ptr/2);
  			//*ch1_ptr--;
  			//*ch1_ptr = (*Rdelptr/2) + (*R_ptr/2);
  			//int32 yeet = *ch1_ptr;
  			//*ch1_ptr = (Uint32)(*ch1_ptr & 0xFFFFFFFF);
  			//McbspaRegs.DXR2.all = (Uint16)((0xFFFF0000 & *ch1_ptr) >> 16);
  			//McbspaRegs.DXR1.all = 0x0000FFFF & *ch1_ptr;
  			ch1_ptr++;
  			*R_ptr = *ch1_ptr;
...
interrupt void local_D_INTCH1_ISR(void)		// DMA Ch1 - McBSP-A Rx
{
	...
	{
	//McbspaRegs.DXR2.all = 0xAAAA;
	//McbspaRegs.DXR1.all = 0xAAAA;
    ...
    if(DmaRegs.CH1.DST_ADDR_SHADOW == ping_buff_offset)
	{
		DmaRegs.CH1.DST_ADDR_SHADOW = pong_buff_offset;
  		...
  		ch1_ptr = &ping_buffer[0]; // set channel pointer to the previous ping/pong buffer because that is where the data was just recorded
	...
	else
	{
		DmaRegs.CH1.DST_ADDR_SHADOW = ping_buff_offset;
  		...
  		ch1_ptr = &pong_buffer[0]; // set channel pointer to the previous ping/pong buffer because that is where the data was just recorded
	}
