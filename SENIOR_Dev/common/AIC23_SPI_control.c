...
void aic23_init(int mic, int i2s_mode)
{
	...
	mcbsp_xmit (command);
    command = linput_volctl(0x1F);    // Unmute left line input and maintain default volume (LIV)
	mcbsp_xmit (command);
	command = rinput_volctl(0x1F);    // Unmute right line input and maintain default volume (RIV)
	mcbsp_xmit (command);
	...
	if (i2s_mode == 1) {
	    ...
	    mcbsp_xmit (command);
	    command = USBsampleratecontrol (SR48); // 12 MHz USB clock -  48 kHz sample rate in USB mode //default = SR48
	    mcbsp_xmit (command);
    }
