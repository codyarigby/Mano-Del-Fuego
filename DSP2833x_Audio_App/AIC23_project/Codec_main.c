...
Uint32 audio_buffer[1024];  	// Note that Uint32 is used, not Uint16

Uint32 * L_channel = &audio_buffer[0];					// This pointer points to the beginning of the L-C data in either of the buffers
Uint32 * R_channel = &audio_buffer[512];				// This pointer points to the beginning of the R-C data in either of the buffers

Uint32 * buffptrL;
Uint32 * buffptrR;

Uint32 audio_buff_offset = (Uint32) &audio_buffer[0];   //
Uint32 output_offset = (Uint32) &output[0];
...
void main(void)
{
   ...
   r_flag = 0; // initialize the interrupt dummyflag
   EALLOW;

   // pointers for both audio channels
   buffptrL = L_channel;
   buffptrR = R_channel;
...
/* Fill the buffer with dummy data */
	...
	//sfor(k=0; k<1024; k++) { pong_buffer[k] = 0xDEADDEAD; }
...
// Step 4. Initialize the Peripherals
	...
	audio_buff_offset++;    // Start at location 1 (32-bit r/w from loc. 1, then 0)init_mcbsp_spi();
	output_offset++;

	// Initialize McBSP-B as SPI Control
...
/* Reassign ISRS */
	...
	PieVectTable.MRINTA = &local_MRINTA_ISR;
...
// The interrupt can be asserted in the following interrupt lines
	...
	PieCtrlRegs.PIEIER6.bit.INTx5 = 1;	// Enable INTx.5 of INT6 (MBSP RA )
...
/* Wait for data */
  	...
  	while(1) {
  		...
  		if(r_flag)
  		{
  			...
  			buffptrL++;
  			buffptrR++;
  			if(buffptrL > &audio_buffer[511])
  			{
  				buffptrL = L_channel;
  			}
  			if(buffptrR > &audio_buffer[1023])
  			{
  				buffptrR = R_channel;
  			}
  			r_flag=0;

  			//write audio effect processing here for left channel
  			/*
  			output = *buffptr;
  			buffptr++;
  			if(buffptr > &audio_buffer[511])
  			{
  				buffptr = L_channel;
  			}
  			EALLOW;
  			while (McbspaRegs.SPCR2.bit.XRDY == 0);
  			McbspaRegs.DXR2.all = (Uint16)((0xFFFF0000 & output ) >> 16);
			McbspaRegs.DXR1.all = (Uint16)(0x0000FFFF & output );
			EDIS;
			r_flag=0;
			*/
  		}
...
// INT6.1
// McBSP MREVTA
interrupt void local_MRINTA_ISR(void)
{
	EALLOW;
	while(DmaRegs.CH1.CONTROL.bit.BURSTSTS);
	r_flag=1;

	//acknowledge interrupts from this pie group
...
/*
// McBSP MXEVTA
interrupt void local_MXINTA_ISR(void)
{
	EALLOW;
	//McbspaRegs.DRR2.all
...
void init_dma()
{
  ...
  DmaRegs.CH1.DST_ADDR_SHADOW = audio_buff_offset;               // First write to audio[1]
  DmaRegs.CH1.DST_BEG_ADDR_SHADOW = audio_buff_offset;
  ...
  DmaRegs.CH1.MODE.bit.CHINTE = 0;          // disable DMA channel interrupts
  ...
  DmaRegs.CH2.TRANSFER_SIZE = 0xFFFF;	// DMA Interrupt every 1024 (n+1) 16-bit words. McBSP still handles 16-bit data only in registers

  DmaRegs.CH2.SRC_TRANSFER_STEP = 3; // move up to the right output from the left output
  DmaRegs.CH2.DST_TRANSFER_STEP = -1;   // Decrement dest. address by 1 (DXR1 back to DXR2) after processing a burst of data

  DmaRegs.CH2.SRC_ADDR_SHADOW = output_offset;               // First read from output[1]
  DmaRegs.CH2.SRC_BEG_ADDR_SHADOW = output_offset;
  ...
  DmaRegs.CH2.MODE.bit.CHINTE = 0;          // Enable DMA channel interrupts
...
void init_mcbspa()
{
    ...
    McbspaRegs.SPCR1.bit.RJUST = 2;		// left-justify word in DRR and zero-fill LSBs
 	...
 	McbspaRegs.MFFINT.bit.RINT = 1;		// Disable all interrupts
